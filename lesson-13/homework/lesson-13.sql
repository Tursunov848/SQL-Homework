-- Creating database for homework13

Create database homerowk_13
Use homerowk_13
...
create table Employees (EMPLOYEE_ID int, FIRST_NAME varchar(50), LAST_NAME varchar(50),EMAIL varchar(255), 
PHONE_NUMBER varchar(50),HIRE_DATE date,JOB_ID varchar(50),SALARY float,COMMISSION_PCT float, MANAGER_ID int,
DEPARTMENT_ID int)
...
create table Students( StudentID int, FullName varchar(100), Grade decimal(5,2))

Select * from Students

insert into Students values 
(1,'James Alexander Hamilton',98.5), (2,'Arthur George Mason',78.2), 
(3,'Lily B Peters',81), (4,'Kayne Thomas Derek',87.3), 
(5,'April Dukane Paris',79.1), (6,'Payton Carl Johnson',97.1)

Select * from Students

-- 3rd table

CREATE TABLE Employee (EmployeeID INTEGER PRIMARY KEY, ManagerID INTEGER NULL, JobTitle VARCHAR(100) NOT NULL )
...
(1001,1,'CA',340),(1001,2,'TX',950),(1001,3,'TX',670), (1001,4,'TX',860),
(2002,5,'WA',320),(3003,6,'CA',650), (3003,7,'CA',830),(4004,8,'TX',120); 

Select * from Orders

-- 5th table

CREATE TABLE Ungroup (ProductDescription VARCHAR(100) PRIMARY KEY, Quantity INTEGER NOT NULL )
...
CREATE TABLE DMLTable (SequenceNumber INTEGER PRIMARY KEY, String VARCHAR(100) NOT NULL)

Select * from DMLTable

INSERT INTO DMLTable (SequenceNumber, String) VALUES 
(1,'SELECT'), (2,'Product,'), (3,'UnitPrice,'), (4,'EffectiveDate'), (5,'FROM'), 
(6,'Products'), (7,'WHERE'), (8,'UnitPrice'), (9,'> 100'); 

Select * from DMLTable

-- 7th table

CREATE TABLE Equations (Equation VARCHAR(200) PRIMARY KEY, TotalSum INTEGER NULL)
...
CREATE TABLE Student (StudentName VARCHAR(50) PRIMARY KEY, Birthday DATE )

Select * from Student

INSERT INTO Student (StudentName, Birthday) VALUES 
('Susan', '2015-04-15'), ('Tim', '2015-04-15'), ('Jacob', '2015-04-15'), 
('Earl', '2015-02-05'), ('Mike', '2015-05-23'), ('Angie', '2015-05-23'), 
('Jenny', '2015-11-19'), ('Michelle', '2015-12-12'), ('Aaron', '2015-12-18')
...
(2001,10,101,87), (2002,103,19,88), (2003,21,23,89), (2004,27,28,91) 

Select * from TestMax

-- 10th table

CREATE TABLE cinema (id INT PRIMARY KEY, movie VARCHAR(50), description VARCHAR(255), rating DECIMAL(3, 1))
...
CREATE TABLE SingleOrder (Id INT, Vals VARCHAR(10)) 

Select * from SingleOrder

INSERT INTO SingleOrder VALUES 
(0,'All'), (1,'Pawan'), (2,'Avtaar'), (3,'Kishan'), (4,'Vaibhav'), (5,'Ashutosh') 

Select * from SingleOrder

-- 12th table

CREATE TABLE person (id INT, ssn VARCHAR(13), passportid VARCHAR(10), itin VARCHAR(10))

Select * from person

INSERT INTO person VALUES 
(1, '111-11-1111', 'aa155', '123itin'), (2, NULL, 'aa156', '124itin'), (3, NULL, NULL, '125itin'), 
(4, NULL, NULL, NULL), (5, '222-22-2222', NULL, '126itin'), (6, '333-33-3333', 'aa157', NULL);
...
CREATE TABLE Personal (SpacemanID INTEGER PRIMARY KEY, JobDescription VARCHAR(100) NOT NULL, MissionCount INTEGER NOT NULL)

Select * from Personal

INSERT INTO Personal (SpacemanID, JobDescription, MissionCount) VALUES 
(1001,'Astrogator',6),(2002,'Astrogator',12),(3003,'Astrogator',17), (4004,'Geologist',21),
(5005,'Geologist',9),(6006,'Geologist',8), (7007,'Technician',13),(8008,'Technician',2),(9009,'Technician',7)
...
Select concat(Employee_id, '-', First_Name, ' ', Last_name) Employees
From employees

-- Task 2
--Select * from Employees

Select replace(phone_number, 124, 999) Phone_Number from employees

-- Task 3
--Select * from Employees

Select first_name Name, len(first_name) length from Employees
...
Select Employee_id, sum(salary) Salary from Employees
...
Select year1,
(select max(v) from (values (Max1), (max2), (max3) values(v)) from TestMax
...
round(datediff(day, Hire_Date, getdate()) / 365.25, 2) Years_of_service From employees
Where round(datediff(day, Hire_Date, getdate()) / 365.25, 2) > 10
and round(datediff(day, hire_Date, getdate()) / 365.25, 2) < 15

-- Task 10
--Select * from Employees

Select Employee_ID, First_Name, Last_Name, Department_ID, Salary from Employees e
...
charindex(' ', fullname) + 1) - charindex(' ', fullname) - 1) else NULL
End MiddleName,

substring(fullname, 
charindex(' ', fullname + ' ', charindex(' ', fullname) + 1) + 1, len(fullname)) LastName
From students

-- Task 13
--Select * from Orders

Select OrderID, CustomerID, DeliveryState from Orders 
Where CustomerID in (
...
Select u1.ProductDescription, sum(u1.quantity) TotalQuantity from Ungroup u1
cross Join Ungroup u2 
Group by u1.ProductDescription

-- Task 15
--Select * from DMLTable

Select top 1 Concat_ws(' ', 'Select', 'Product', 'Unitprice', 'EffectiveDate', 'FROM', 'Products', 'Where', 'UnitPrice', '>100') name
From DMLTable
...
Select Employee_id, concat(First_name, ' ', Last_name) Fullname, Salary from Employees
Where concat(First_name, ' ', Last_name) like '%a%' and cast(Salary as int) % 5 = 0

-- Task 19
--Select * from Employees

Select Department_ID, count(*) TotalEmployees,
sum(case when datediff(year, Hire_Date, getdate()) > 3 then 1 else 0 end) Workingyears,
(sum(case when datediff(year, Hire_Date, getdate()) > 3 then 1 else 0 end) * 100.0) / count(*) Percentage
From Employees
...
--MostExperienced:
Select top 1 SpacemanId, sum(Missioncount) MostExperienced from personal
Group by SpacemanId
Order by sum(Missioncount) desc

--LeastExperienced:
Select top 1 SpacemanId, sum(Missioncount) LeastExperienced from personal
Group by SpacemanId
Order by sum(Missioncount) asc
