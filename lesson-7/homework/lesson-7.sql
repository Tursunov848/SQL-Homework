...
(EmployeeID INT PRIMARY KEY, FirstName VARCHAR(50) NULL, 
LastName VARCHAR(50) NULL, DepartmentName VARCHAR(50), 
Salary DECIMAL(10, 2), HireDate DATE, Age INT, 
Email VARCHAR(100) NULL, Country VARCHAR(50) );

Select * from Employees

Insert Employees (EmployeeID, FirstName, LastName, DepartmentName, Salary, HireDate, Age, Email, Country) 
Values 
...
(1, 1, 2, '2023-05-14', 1, 800.00), (2, 1, 3, '2024-09-07', 2, 800.00), 
(3, 1, 4, '2022-11-22', 1, 250.00), (4, 1, 5, '2021-03-30', 3, 150.00),
...
-- Task 1: find the minimum (MIN) price of a product in the Products table.

Select min(price) as Price from Products

-- Task 2: find the maximum (MAX) Salary from the Employees table.
...
-- Task 4: count the number of unique product categories
--Select * from Products

Select count(distinct Category) as Category from Products

-- Task 5: find the total (SUM) sales amount for the product with id 7 in the Sales table.
--Select * from Sales

Select sum(Saleamount) as Sales from Sales
Where ProductID = 7

-- Task 6: calculate the average (AVG) age of employees in the Employees table.
--Select * from Employees

Select avg(age) as Age from Employees

-- Task 7: use GROUP BY to count the number of employees in each department.
--Select * from Employees

Select DepartmentName, count(EmployeeID) as Number from employees
...
-- Task 9: calculate the total (SUM) sales per Customer in the Sales table.
--Select * from Sales

Select CustomerID, sum(SaleAmount) as Sales from Sales
Group by CustomerID 

-- Task 10: use HAVING to filter departments...
--Select * from Employees

Select DepartmentName, count(EmployeeID) as Dep from employees
Group by DepartmentName
Having count(EmployeeID) > 5

-- Task 11: calculate the total sales and average sales
--Select * from sales

Select ProductId, sum(saleamount) as Total, avg(Saleamount) as Average from Sales
Group by ProductId

-- Task 12: uses COUNT(columnname) to count the number of employees from the Department HR.
--Select * from Employees

Select count(EmployeeID) as Employees from Employees
Where DepartmentName = 'HR'

-- Task 13:  finds the highest (MAX) and lowest (MIN) Salary by department..
--Select * from Employees

Select DepartmentName, min(Salary) as Min, max(Salary) as Max from Employees
Group by DepartmentName

-- Task 14: use GROUP BY to calculate the average salary per Dep

Select DepartmentName, avg(salary) average from Employees
Group By DepartmentName

-- Task 15: show the AVG salary and COUNT(*) of employees working in each dep
--Select * from Employees

Select DepartmentName, avg(salary) average, count(*) as Total from Employees
Group by DepartmentName

-- Task 16: uses HAVING to filter product categories with an average price greater than 400.
--Select * from Products
...
-- Task 17: calculates the total sales for each year...
--Select * from Sales

Select Year(SaleDate) as year, sum(SaleAmount) as Sales from Sales
Group by Year(SaleDate)


-- Task 18: use COUNT to show the number of customers who placed at least 3 orders.
--Select * from Orders

Select CustomerID, count(Quantity) as Quantity from Orders
Group by CustomerID
Having count(Quantity) >= 3

-- Task 19: HAVING clause to filter out Departments with total salary expenses greater than 500,000.
--Select * from employees

Select DepartmentName, sum(Salary) as Total from Employees
Group by DepartmentName
Having sum(Salary) > 500000.00

-- Task 20: average (AVG) sales for each product category...
--Select * from Sales

Select ProductID, avg(Saleamount) as Sales from Sales
Group by ProductID
Having avg(Saleamount) > 200.00

-- Task 21: calculate the total (SUM) sales for each Customer...
--Select * from Sales

Select CustomerID, sum(SaleAmount) as Sales from Sales
Group by CustomerID
Having sum(SaleAmount) > 1500.00

-- Task 22:  find the total (SUM) and average (AVG) salary of employees...
--Select * from employees

Select DepartmentName, sum(Salary) as Total, avg(Salary) as Average from employees
Group by DepartmentName
Having avg(Salary) > 65000.00

-- Task 23: find the maximum (MAX) and minimum (MIN) order value for each customer..
--Select * from Orders

Select CustomerID, max(TotalAmount) as Max, min(Totalamount) as Min from Orders
Group by CustomerID
Having min(Totalamount) >= 50.00 


-- Task 24: calculates the total sales (SUM) and counts distinct products sold in each month..
--Select * from Orders

Select month(Orderdate) as EachMonth, sum(TotalAmount) as Total, count(distinct ProductID) as ProductSold from Orders
Group by month(Orderdate)
Having count(distinct ProductID) > 8

-- Task 25: find the MIN and MAX order quantity per Year
--select * from Orders

Select year(orderdate) as Year, min(Quantity) as Min, Max(Quantity) as Max from Orders
